...
const passwordResetTemplate =
  hbs.compile(
    fs.readFileSync(
      path.join(__dirname, "../../../mail/passwordReset.hbs"), "utf-8"));

/**
 * Internal helper function to send a message using the mailgun API
 * and also convert the API to promise format.
 *
 * @param data a object to pass to mailgun to send ({ to, from, subject, html }
 *             and possibly more -- see Mailgun.messages.SendData)
 */
