...
.get((req: Request, res: Response, next: NextFunction) => {
	...
	Event.getPastEvents(offset, limit).then(events: any[] => {
...
.get((req: Request, res: Response, next: NextFunction) => {
	const offset: number = parseInt(req.query.offset);
	const limit: number = parseInt(req.query.limit);
	Event.getFutureEvents(offset, limit).then(events: any[] => {
		res.json({ error: null, events: events.map(e => e.getPublic()) });
	}).catch(next);
});
...
.get((req: Request, res: Response, next: NextFunction) => {
	...
	if (!req.params.uuid || !req.params.uuid.trim()) {
		const offset: number = parseInt(req.query.offset);
		const limit: number = parseInt(req.query.limit);
		Event.getAll(offset, limit).then(events: any[] => {
	...
	// CASE: UUID is present, should return matching event
		...
		Event.findByUUID(req.params.uuid).then(event: any => {
...
.post((req: Request, res: Response, next: NextFunction) => {
	...
	Event.create(Event.sanitize(req.body.event)).then(event: any => {
...
.delete((req: Request, res: Response, next: NextFunction) => {
	if (!req.params.uuid)
		return next(new error.BadRequest());
	Event.destroyByUUID(req.params.uuid).then(numDeleted: any => {
